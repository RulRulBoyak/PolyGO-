...
public class SearchActivity extends AppCompatActivity {
    ...
    private void filter(){String q=search.getText().toString().trim().toLowerCase(Locale.ROOT);String cat=category.getSelectedItem()==null?"":category.getSelectedItem().toString();List<AppDataStore.ProductRecord> filtered=new ArrayList<>();for(AppDataStore.ProductRecord p:all){boolean text=q.isEmpty()||p.title.toLowerCase(Locale.ROOT).contains(q)||p.seller.toLowerCase(Locale.ROOT).contains(q)||p.description.toLowerCase(Locale.ROOT).contains(q);boolean categoryMatch=cat.startsWith("All")||p.category.equalsIgnoreCase(cat);if(text&&categoryMatch)filtered.add(p);} adapter=new ProductCardAdapter(filtered,(a,p)->{android.content.Intent i=new android.content.Intent(this,ProductDetailActivity.class);i.putExtra(ProductDetailActivity.EXTRA_LISTING_ID,p.id);startActivity(i);});((RecyclerView)findViewById(R.id.rvSearchResults)).setAdapter(adapter);count.setText(filtered.size()+" listings");empty.setVisibility(filtered.isEmpty()?View.VISIBLE:View.GONE);}
}
